...
def hierarchical_clustering(embeddings):
    ...
    # Create a mapping from cluster label to user IDs
    cluster_mapping = {}
...
def main():
    dataset_dir = 'dataset'
    embeddings_dir = 'Testing/embedding'
    query_image_path = 'eval/aaaaaaaaaa.jpg'
    ...
    # Check if hierarchical clusters exist; if not, compute and save them
    ...
    # Find the most relevant cluster for the query face
    cluster_similarities = {}
    for cluster_id, cluster_user_ids in cluster_mapping.items():
        cluster_embeddings = {user_id: embeddings[user_id] for user_id in cluster_user_ids}
    ...
    # Select the cluster with the highest similarity
    ...
    # Find the user with the highest similarity within the selected cluster
